#!/usr/bin/env bun
// HORIZON hooks/security-validator.ts
...
const ATTACK_PATTERNS = {
  ...
  // Tier 9: HORIZON infrastructure protection - Block
  horizonProtection: {
    patterns: [
      /rm.*\.claude/i,
      /git\s+push.*public/i,
    ...
    action: 'block',
    message: 'BLOCKED: HORIZON infrastructure protection triggered'
  },

  // Tier 10: Raw source protection - Warn
  clientDataAccess: {
    patterns: [
      /cat\s+.*01-source\//i,
      /head\s+.*01-source\//i,
    ...
    action: 'warn',
    message: 'WARNING: Direct access to raw source files in 01-source/ detected. Raw source is never committed and never edited; work from the versioned ledgers in 02-demand/ and 04-supply/ that DataEngineer produces.'
  }
};
