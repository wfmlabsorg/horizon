...
| Task Type | Model | Why |
|-----------|-------|-----|
| Grunt work | `haiku` | 10-20x faster, cheapest |
| Standard analysis | `sonnet` | Balanced speed/quality |
| Deep reasoning | `opus` | Maximum intelligence (Opus 4.6) |

### Opus 4.6 Agent Teams
When `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is set, agents can coordinate as teams:
- **Lead agent** spawns teammates, assigns tasks, synthesizes results
- **Teammates** are separate Claude Code instances with their own context windows
- Coordinate via **shared task list** with dependency tracking (pending/in_progress/completed)
- Teammates can **message each other directly**, not just report back to the lead
- Best for: parallel research, consulting pipelines, multi-stream investigations
