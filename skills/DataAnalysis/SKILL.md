...
| `--output` | Output directory | `~/horizon/books/halcyon/03-output` |
...
project:
  name: "Halcyon Group Interval Analysis"
  client: "Halcyon Group"

source:
  directory: ~/horizon/books/halcyon/01-source/interval-data
  file_pattern: "*.xlsx"
...
output:
  directory: ~/horizon/books/halcyon/03-output
...
/analyze ~/horizon/books/halcyon/01-source/interval-data \
  --type deviation \
  --metric Actual_SL \
  --goals "*:0.80"
```
...
/analyze --config ~/horizon/books/halcyon/analysis_config.yaml
```

### Step-by-Step
```bash
# 1. Ingest and profile sources
/analyze ingest ~/horizon/books/halcyon/01-source
